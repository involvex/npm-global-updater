...
export const validateConfigStructure = (config: ConfigSettings) => {
	...
	if (!('backups' in config)) throw new Error('Config missing backups property')
	...
	if (!('general' in config)) throw new Error('Config missing general property')
...
export const validatePackageMetadataStructure = (
	...
	packages.forEach(pkg => {
		...
		if (!('version' in pkg)) throw new Error('Package missing version property')
...
export const validateAlertHistoryStructure = (alerts: AlertHistory[]) => {
	alerts.forEach(alert => {
		...
		if (!('message' in alert)) throw new Error('Alert missing message property')
		...
		if (typeof alert.id !== 'string') throw new Error('Alert id must be string')
